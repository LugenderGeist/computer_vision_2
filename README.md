...
Так как в дальнейшем планируется использовать метод SIFT, использующийся для одноканальных изображений - переведем изображения в grayscale. Градации серого упрощают вычисления, так как вместо трех каналов (R, G, B) используется один канал интенсивности.
